...
@section('title', 'Panel de administración - Sliders')
...
<div class="row">
    <div class="col-md-6">
        <div class="page-title">
            <h3>Sliders del Landing</h3>
        </div>
    </div>
    <div class="col-md-6 text-right">
        <a class="btn btn-primary" href="{{ route('wp.slider.create') }}"><i class="fa fa-plus" aria-hidden="true"></i>&nbsp;Agregar slider</a>
    </div>
</div>
    ...
    <script>
        var dataTable = $('#tableList').DataTable({
            ...
            columns: [
                ...
                {data:'id', render: function(data, type, row) {
                    var btn = '<a href="{{ url('wpanel/slider') }}/'+row.id+'/edit"><img src="{{ asset('img/wpanel/edit.png') }}"></a>';
                    return btn;
                }, 'orderable': false, 'searchable': false, 'className': "text-center"},
                {data:'id', render: function(data, type, row) {
        ...
        function enabledRow(obj) {
            ...
            enabledRegister('{{ route('wp.slider.enabled') }}', 'id=' + id, 'enabledRow' + id);
        }
        function deleteRow(obj) {
            var id = $(obj).attr('data');
            var row = $(obj).parent().parent('tr');
            eliminateRegister('{{ route('wp.slider.delete') }}', 'id=' + id, row);
        }
    </script>
@stop
    ...
    <script>
        var dataTable = $('#tableList').DataTable({
            ...
            columns: [
                ...
                {data:'id', render: function(data, type, row) {
                    var btn = '<a href="{{ url('wpanel/service') }}/'+row.id+'/edit"><img src="{{ asset('img/wpanel/edit.png') }}"></a>';
                    return btn;
                }, 'orderable': false, 'searchable': false, 'className': "text-center"},
                {data:'id', render: function(data, type, row) {
        ...
        function enabledRow(obj) {
            ...
            enabledRegister('{{ route('wp.service.enabled') }}', 'id=' + id, 'enabledRow' + id);
        }
        function deleteRow(obj) {
            var id = $(obj).attr('data');
            var row = $(obj).parent().parent('tr');
            eliminateRegister('{{ route('wp.service.delete') }}', 'id=' + id, row);
        }
    </script>
@stop
...
@section('js')
    ...
    <script>
        var dataTable = $('#tableList').DataTable({
            ...
            responsive: true,
            order: [[0, 'desc']],
            ajax: "{{ route('wp.specialties.list') }}",
            paging: true,
            pageLength: 25,
            columns: [
                {data:'id'},
                ...
                {data:'id', render: function(data, type, row) {
                    var btn = '<a href="{{ url('wpanel/specialties') }}/'+row.id+'/edit"><img src="{{ asset('img/wpanel/edit.png') }}"></a>';
                    return btn;
                }, 'orderable': false, 'searchable': false, 'className': "text-center"},
                {data:'id', render: function(data, type, row) {
        ...
        function enabledRow(obj) {
            ...
            enabledRegister('{{ route('wp.specialties.enabled') }}', 'id=' + id, 'enabledRow' + id);
        }
        function deleteRow(obj) {
            var id = $(obj).attr('data');
            var row = $(obj).parent().parent('tr');
            eliminateRegister('{{ route('wp.specialties.delete') }}', 'id=' + id, row);
        }
    </script>
@stop
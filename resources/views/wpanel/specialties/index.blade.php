...
@section('title', 'Panel de administración - Especialidades')
...
<div class="row">
    ...
    <div class="col-md-6 text-right">
        <a class="btn btn-primary" href="{{ route('wp.specialties.create') }}"><i class="fa fa-plus" aria-hidden="true"></i>&nbsp;Agregar especialidad</a>
    </div>
</div>
...
@section('js')
    ...
    <script>
        window.WPANEL_SPECIALTIES_INDEX_CONFIG = {
            listUrl: "{{ route('wp.specialties.list') }}",
            enabledUrl: "{{ route('wp.specialties.enabled') }}",
            deleteUrl: "{{ route('wp.specialties.delete') }}",
            editBaseUrl: "{{ url('wpanel/specialties') }}",
            assets: {
    ...
    <script src="{{ asset('js/wpanel/specialties/index.js') }}"></script>
@stop

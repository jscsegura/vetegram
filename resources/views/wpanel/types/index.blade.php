...
<div class="row">
    ...
    <div class="col-md-6 text-right">
        <a class="btn btn-primary" href="{{ route('wp.animal-types.create') }}"><i class="fa fa-plus" aria-hidden="true"></i>&nbsp;Agregar Tipo de animal</a>
    </div>
</div>
...
@section('js')
    ...
    <script>
        var dataTable = $('#tableList').DataTable({
            ...
            responsive: true,
            order: [[0, 'desc']],
            ajax: "{{ route('wp.animal-types.list') }}",
            paging: true,
            pageLength: 25,
            columns: [
                {data:'id'},
                ...
                {data:'id', render: function(data, type, row) {
                    var btn = '<a href="{{ url('wpanel/animal-breed/show') }}/'+row.id+'"><img src="{{ asset('img/wpanel/menu.png') }}"></a>';
                    return btn;
                ...
                {data:'id', render: function(data, type, row) {
                    var btn = '<a href="{{ url('wpanel/animal-types') }}/'+row.id+'/edit"><img src="{{ asset('img/wpanel/edit.png') }}"></a>';
                    return btn;
                }, 'orderable': false, 'searchable': false, 'className': "text-center"},
                {data:'id', render: function(data, type, row) {
        ...
        function enabledRow(obj) {
            ...
            enabledRegister('{{ route('wp.animal-types.enabled') }}', 'id=' + id, 'enabledRow' + id);
        }
        function deleteRow(obj) {
            var id = $(obj).attr('data');
            var row = $(obj).parent().parent('tr');
            eliminateRegister('{{ route('wp.animal-types.delete') }}', 'id=' + id, row);
        }
    </script>
@stop
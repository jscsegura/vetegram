...
<div class="container-fluid">
    <header id="mainHead" class="position-relative">
        <nav class="navbar navbar-expand-lg px-2 px-lg-3 py-3 me-auto">
            ...
            <div id="doctorMenu" class="collapse navbar-collapse justify-content-between">
                <a href="{{ route('dash') }}" class="d-block">
                    <!-- <svg class="dashLogo mt-4 mt-lg-0 mb-2 mb-lg-0" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" viewBox="0 0 1000 224" style="enable-background:new 0 0 1000 224;" xml:space="preserve">
                ...
                <ul class="navbar-nav">
                    ...
                    <li class="nav-item"><a href="{{ route('appointment.index') }}" class="nav-link px-0 px-lg-3 py-1 py-lg-2 text-uppercase {{ ((isset($thismenu))&&($thismenu == 'appointment')) ? 'active' : '' }}"><small>{{ trans('dash.menu.cita') }}</small></a></li>
                    <li class="nav-item"><a href="{{ route('pet.owners') }}" class="nav-link px-0 px-lg-3 py-1 py-lg-2 text-uppercase {{ ((isset($thismenu))&&($thismenu == 'owners')) ? 'active' : '' }}"><small>{{ trans('dash.menu.owners') }}</small></a></li>
                    <li class="nav-item dropdown">
                        <a class="nav-link px-0 px-lg-3 py-1 py-lg-2 text-uppercase dropdown-toggle" href="#" role="button" data-bs-toggle="dropdown" aria-expanded="false">
                          <small>{{ trans('dash.menu.billing') }}</small>
                        </a>
                        <ul class="dropdown-menu ms-lg-3">
                          <li><a class="dropdown-item small" href="{{ route('invoice.index') }}">{{ trans('dash.menu.bills') }}</a></li>
                          <li><a class="dropdown-item small" href="{{ route('invoice.uptake') }}">{{ trans('dash.menu.accept.invoice') }}</a></li>
                          <li><a class="dropdown-item small" href="{{ route('invoice.proform') }}">{{ trans('dash.menu.proform') }}</a></li>
                        </ul>
                    </li>              
                </ul>
                <div class="d-none d-lg-block" aria-hidden="true"></div>
            </div>
        </nav>
        <div id="circleIcons" class="d-flex flex-wrap align-items-center me-2 me-lg-3 position-absolute end-0 top-0">
            @if (Config::get('app.locale') == 'en')
            <a href="{{ route('change.language', ['es'])}}" class="btn btn-outline-secondary btn-sm text-uppercase me-1">Es</a>
            @endif
            @if (Config::get('app.locale') == 'es')
            <a href="{{ route('change.language', ['en'])}}" class="btn btn-outline-secondary btn-sm text-uppercase me-1">En</a>
            @endif
            <a href="{{ route('notifications') }}" class="btn btn-link btn-sm d-flex {{ (request()->is('notifications')) ? 'active' : '' }}"><span class="m-auto position-relative"><i class="fa-solid fa-bell">@if(session()->get('notifieds') > 0)<em class="bellPoint"></em>@endif</i></span></a>

            <div class="dropdown">
                <button class="btn btn-secondary p-0 cyanIcon rounded-pill dropdown-toggle" type="button" data-bs-toggle="dropdown" data-bs-offset="0,10" aria-expanded="false">
                    <i class="fa-solid fa-user"></i>
                </button>
                <ul class="dropdown-menu">
                  <li><a class="dropdown-item small" href="{{ route('profile') }}">{{ trans('dash.menu.profile') }}</a></li>
                  @if(Auth::guard('web')->user()->rol_id == 3)
                  <li><a class="dropdown-item small" href="{{ route('plan') }}">{{ trans('dash.menu.myplan') }}</a></li>
                    <li><a class="dropdown-item small" href="{{ route('admin') }}">{{ trans('dash.menu.setting') }}</a></li>
                  @else
                    <li><a class="dropdown-item small" href="{{ route('sett.index') }}">{{ trans('dash.menu.setting') }}</a></li>
                  @endif
                  <li><hr class="dropdown-divider"></li>
                  <li><a class="dropdown-item small" href="{{ route('login.logout') }}">{{ trans('auth.btn.logout') }}</a></li>
                </ul>
            </div>
        </div>
    </header>

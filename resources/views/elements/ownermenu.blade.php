    ...
    <header id="mainHead" class="position-relative">
        <nav class="navbar navbar-expand-lg px-2 px-lg-3 py-3 me-auto">
            ...
            <div id="doctorMenu" class="collapse navbar-collapse justify-content-between">
                ...
                <ul class="navbar-nav @if((Auth::guard('web')->check())&&(Auth::guard('web')->user()->rol_id == 8)) @else hideUser @endif">
                    <li class="nav-item"><a href="{{ route('dash') }}" class="nav-link px-0 px-lg-3 py-1 py-lg-2 text-uppercase {{ ((isset($thismenu))&&($thismenu == 'dashowner')) ? 'active' : '' }}"><small>{{ trans('dash.menu.dashboard') }}</small></a></li>
                    <li class="nav-item"><a href="{{ route('search.index') }}" class="nav-link px-0 px-lg-3 py-1 py-lg-2 text-uppercase {{ ((isset($thismenu))&&($thismenu == 'search')) ? 'active' : '' }}"><small>{{ trans('dash.menu.search') }}</small></a></li>
                    <li class="nav-item"><a href="{{ route('appointment.index') }}" class="nav-link px-0 px-lg-3 py-1 py-lg-2 text-uppercase {{ ((isset($thismenu))&&($thismenu == 'appointowner')) ? 'active' : '' }}"><small>{{ trans('dash.menu.cita') }}</small></a></li>
                    <li class="nav-item"><a href="{{ route('pets.index') }}" class="nav-link px-0 px-lg-3 py-1 py-lg-2 text-uppercase {{ ((isset($thismenu))&&($thismenu == 'pets')) ? 'active' : '' }}"><small>{{ trans('dash.menu.pets') }}</small></a></li>
                </ul>
                <div class="d-none d-lg-block" aria-hidden="true"></div>
            </div>
        </nav>
        
        <div id="circleIcons" class="d-flex flex-wrap align-items-center me-2 me-lg-3 position-absolute end-0 top-0 @if((Auth::guard('web')->check())&&(Auth::guard('web')->user()->rol_id == 8)) @else hideUser @endif">

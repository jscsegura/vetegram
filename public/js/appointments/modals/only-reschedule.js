...
(function() {
    var cfg = window.APPOINT_MODAL_CONFIG && window.APPOINT_MODAL_CONFIG.onlyReschedule;
    if (!cfg) return;
    ...
    function initDateDropper() {
        ...
        new dateDropper({
            ...
            onDropdownExit: getHoursAppointmentToReschedule
        });
    }

    function setIdAppointmentToOnlyReschedule(id, userId) {
        ...
        if (dateModalCancel) dateModalCancel.style.display = 'block';
    }
    Appointments.setIdAppointmentToOnlyReschedule = setIdAppointmentToOnlyReschedule;

    function getHoursAppointmentToReschedule() {
        var userField = getEl(ids.userField, 'IdUserAppointmentToCancel');
    ...
    Appointments.getHoursAppointmentToReschedule = getHoursAppointmentToReschedule;

    function reserverHourAppointmentToReschedule(select) {
        ...
        request.done(function(data) {
            if (data.reserve !== '1') {
                ...
                if (typeof getHours === 'function') {
                    ...
                    getHoursAppointmentToReschedule();
                }
            }
        });
    }
    Appointments.reserverHourAppointmentToReschedule = reserverHourAppointmentToReschedule;
    ...
    function confirmSaveActionReschedule() {
        ...
        var userId = userField ? userField.value : '';
        var date = dateField ? dateField.value : '';
        var time = hourSelect ? hourSelect.value : '';
        ...
        var title = labels.titleReschedule || '';
        var text = labels.textReschedule || '';
        var btn = labels.btnReschedule || '';
    ...
    document.addEventListener('change', function(e) {
        if (e.target && e.target.matches('[data-appoint-action="only-reschedule-hour"]')) {
            reserverHourAppointmentToReschedule(e.target);
        }
    });

    document.addEventListener('click', function(e) {
        var button = e.target.closest('[data-appoint-action="only-reschedule-save"]');
        if (button) {
            confirmSaveActionReschedule();
        }
    });
})();

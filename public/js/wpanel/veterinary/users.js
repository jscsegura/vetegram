(function() {
    const cfg = window.WPANEL_VETERINARY_USERS_CONFIG || {};
    ...
    function showPlus(obj) {
        const plus = $(obj).attr('data-plus');
        if (plus == '0') {
            $(obj).attr('data-plus', '1');
            $('#btnplus').html(cfg.hideDetailsLabel || '<i class="fa fa-minus" aria-hidden="true"></i>&nbsp;Ocultar detalles');
            $('#moreDetails').show(1000);
        } else {
            $(obj).attr('data-plus', '0');
            $('#btnplus').html(cfg.showDetailsLabel || '<i class="fa fa-plus" aria-hidden="true"></i>&nbsp;Ver m\u00e1s detalles');
            ...
            { data: 'id' },
            {
                ...
                render: function(data, type, row) {
                    switch (row.rol_id) {
                        ...
                        case 6: return 'Groomer';
                        case 7: return 'Contador';
                        default: return 'NA';
                    }
                },
                orderable: false,

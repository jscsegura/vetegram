(function() {
    ...
    const config = window.SCHEDULE_EXCEPTION_INDEX_CONFIG || {};
    ...
    function removeExceptionAvailability(id) {
    ...
    document.addEventListener('click', function(e) {
        const searchBtn = e.target.closest('[data-schedule-action="exception-search"]');
        if (searchBtn) {
            e.preventDefault();
            searchRows();
            return;
        }
        const deleteBtn = e.target.closest('[data-schedule-action="exception-delete"]');
        if (deleteBtn) {
            e.preventDefault();
            const rowId = deleteBtn.getAttribute('data-row-id');
            if (rowId) {
                removeExceptionAvailability(rowId);
            }
        }
    });
})();
